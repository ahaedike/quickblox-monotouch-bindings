...
namespace QuickBlox {
	...
	public unsafe partial class FileParameter : NSObject {
		...
		[CompilerGenerated]
		public virtual NSObject Type {
			[Export ("type")]
			get {
				NSObject ret;
				if (IsDirectBinding) {
					ret =  Runtime.GetNSObject<NSObject> (MonoTouch.ObjCRuntime.Messaging.IntPtr_objc_msgSend (this.Handle, selTypeHandle));
				} else {
					ret =  Runtime.GetNSObject<NSObject> (MonoTouch.ObjCRuntime.Messaging.IntPtr_objc_msgSendSuper (this.SuperHandle, selTypeHandle));
				}
				MarkDirty ();
				__mt_Type_var = ret;
				return ret;
			...
			set {
				if (value == null)
					throw new ArgumentNullException ("value");
				if (IsDirectBinding) {
					MonoTouch.ObjCRuntime.Messaging.void_objc_msgSend_IntPtr (this.Handle, selSetType_Handle, value.Handle);
				} else {
					MonoTouch.ObjCRuntime.Messaging.void_objc_msgSendSuper_IntPtr (this.SuperHandle, selSetType_Handle, value.Handle);
				}
				MarkDirty ();
				__mt_Type_var = value;
			}
